	...
	class GSPB_Postfilters
	{
		...
		public function extract_filters()
		{
			...
			if ($filter_args->data_source == 'ids' && $filter_args->ids != '') {
				...
				//$args['search_filter_query_id'] = true;
				if (is_category()) {
					$args['post_type'] = 'post';
...
// Filter panel render
//////////////////////////////////////////////////////////////////
function gspb_custom_taxonomy_dropdown($taxdrop, $limit = '40', $class = '', $taxdroplabel = '', $containerid = '', $taxdropids = '')
{
	$args = array(
		'taxonomy' => $taxdrop,
		'number' => $limit,
	...
	if ($terms && !is_wp_error($terms)) {
		...
		if (empty($taxdroplabel)) {
			$taxdroplabel = esc_html__('Choose category', 'greenshift');
		}
		$output .= '<li class="label"><span class="gspb_tax_placeholder">' . $taxdroplabel . '</span><span class="gspb_choosed_tax"></span></li>';
		$output .= '<li class="gspb_drop_item"><span data-sorttype="" class="gspb_filtersort_btn" data-containerid="' . $containerid . '">' . esc_html__('All categories', 'greenshift') . '</span></li>';
		foreach ($terms as $term) {
...
function gspb_vc_filterpanel_render($filterpanel = '', $containerid = '', $taxdrop = '', $taxdroplabel = '', $taxdropids = '', $filterheading = '')
{
	if (!$filterpanel) {
		return;
	}
	...
	if (!empty($filterpanel[0])) {
		...
		if ($taxdrop) {
			...
			$output .= gspb_custom_taxonomy_dropdown($taxdrop, '40', 'gspb_tax_dropdown', $taxdroplabel, $containerid, $taxdropids);
			$output .= '</div>';
		}
