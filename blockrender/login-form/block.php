...
class LoginForm{
    ...
    public function gspb_login_form_validation() {
        global $wp;
        ...
        if( !check_ajax_referer( 'ajax-gspb-login-nonce', 'gspb-login-form', false) ){
            echo json_encode(array('error' => true, 'message'=> '<div class="error"><i></i>'.__('Session has expired, please reload the page and try again', 'greenshift').'</div>'));
        }

        // Check if input variables are empty
        elseif(empty($user_login) or empty($user_pass)){
            echo json_encode(array('error' => true, 'message'=> '<div class="error"><i></i>'.__('Please fill all form fields', 'greenshift').'</div>'));
        }
            ...
            if(is_wp_error($user)){
                ...
                echo json_encode(array('error' => false, 'message'=> '<div class="success">'.__('Login successful, reloading page...', 'greenshift').'</div>'));
            }
        }
        wp_die();
    }
    ...
    public function render_block($settings = array(), $inner_content=''){
        ...
        if(!$userid) {
            ...
            <form method="POST" class="gspb-login-form">
                ...
                <?php if(!empty($dont_have_acc_text) || !empty($dont_have_acc_link_text)):?>
                    <div class="dont-have-account">
                        ...
                        <a
                                href="<?php echo $dont_have_acc_link_url_res?>"
                        >
            ...
            echo __('You are already logged in. ', 'greenshift');
            echo '<a href="' . esc_url(wp_logout_url(home_url( $wp->request ))) . '">' . __('Logout?', 'greenshift') . '</a><br />';
