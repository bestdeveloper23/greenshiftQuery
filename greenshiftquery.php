...
define('GREENSHIFTQUERY_PLUGIN_VER', '3.7.1');
...
function greenshiftquery_push_update($transient)
{
    ...
    if (!is_wp_error($remote) && $remote) {
        ...
        if ($remote && version_compare(GREENSHIFTQUERY_PLUGIN_VER, $remote->version, '<') && version_compare($remote->requires, get_bloginfo('version'), '<')) {
            $res = new stdClass();
...
if (!function_exists('greenShiftQuery_block_script_assets')) {
    function greenShiftQuery_block_script_assets($html, $block)
    {
        ...
        if (!is_admin()) {
            ...
            if ($blockname == 'greenshift-blocks/thumbcounter') {
            ...
            } else if ($blockname === 'greenshift-blocks/advanced-listing' || $blockname === 'greenshift-blocks/querygrid') {
                ...
                if (!empty($block['attrs']['enable_pagination']) && ($block['attrs']['enable_pagination'] == '2' || $block['attrs']['enable_pagination'] == '3')) {
...
if (!function_exists('greenShiftQuery_editor_assets')) {
    function greenShiftQuery_editor_assets()
    {
        ...
        wp_set_script_translations('greenShiftQuery-block-js', 'greenshiftquery');
...
function filter_by_custom_meta($compare, $key = '', $value = '', $not_show_for_selected = false)
{
...
function filter_by_cpt($post_type, $tax_name, $tax_slug = [], $tax_slug_exclude = [], $cat = [], $cat_exclude = [], $tag = [], $tag_exclude = [], $price_range = '', $type = 'all', $not_show_for_selected = false, $hasParent = false)
{
    $postId = get_the_ID();
        ...
        if ($query_by === 'post_type') {
            switch ($dataSource) {
                ...
                case 'cat':
                    ...
                    break;
                case 'cpt':
                    $isVisibility = filter_by_cpt($post_type, $tax_name, $tax_slug, $tax_slug_exclude, $cat, $cat_exclude, $tag, $tag_exclude, $price_range, $type, $not_show_for_selected, $hasParent);
                    break;
                default:
                    break;
            }
...
add_filter( 'load_script_translation_file', 'greenshiftquery_custom_block_fix_translation_location', 10, 3 );
function greenshiftquery_custom_block_fix_translation_location( string $file, string $handle, string $domain ): string {
	if ( strpos( $handle, 'greenShiftQuery-block-js' ) !== false && 'greenshiftquery' === $domain ) {
